...
BiblioH* meme_auteur(BiblioH *b,char* auteur){
    BiblioH* B = creer_biblioH(1);
    int s = 0;
    for(int i=0;i<b->m;i++){
        if(fonctionClef(auteur) == (b->T)[i]->clef){ // on parcourt la liste chaine avec le meme clef de notre auteur
            LivreH *l = (b->T)[i];
            while(l != NULL){
...
BiblioH* fusion(BiblioH *b1,BiblioH *b2){
    ...
    BiblioH *b = creer_biblioH(b1->m+b2->m);
        ...
        while(lc != NULL){
            ...
            while(ld != NULL){
                if((strcmp(ld->titre,lc->titre) == 0)&&(ld->num != lc->num)&&(strcmp(lc->auteur,ld->auteur) == 0)){ // il y a deux ouvrages identiques
                    if(l == NULL){
                        l = creer_livreH(ld->num,ld->titre,ld->auteur);
                    }else{
                    	...
                    	lm->suivant = creer_livreH(ld->num,ld->titre,ld->auteur);
                    }
                }
                ld = ld->suivant;
            }
